...
if ( $_SERVER["REQUEST_METHOD"] == "POST" and !isset($_POST['exportBtn']) 
	...
	if(isset($_POST['exportBtn'])){
		gc_collect_cycles();
		unlink('/opt/lampp/htdocs/databaseFile.csv');
		$query = "SELECT * INTO OUTFILE '/opt/lampp/htdocs/databaseFile.csv'
		...
		if(file_exists('/opt/lampp/htdocs/databaseFile.csv')){
			header('Content-Type: text/csv');
			header('Content-Length: '.filesize('/opt/lampp/htdocs/databaseFile.csv'));
			header('Content-Disposition: attachment; filename='.'dataFile.csv');
			readfile('/opt/lampp/htdocs/databaseFile.csv');
			exit;
		}
...
function logout()
{
	location.replace('http://137.45.241.80/target_hunter/login.php');
}
</script>
